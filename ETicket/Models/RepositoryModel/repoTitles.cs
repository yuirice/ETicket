...
public class z_repoTitles : BaseClass
{
    ...
    /// <returns></returns>
    public List<Titles> GetDapperDataList(string searchText)
    {
    ...
    {
        string str_query = @"SELECT TitleNo, TitleName, Remark FROM Titles ";
        return str_query;
    }
    /// <summary>
    /// 取得 SQL 條件式
    ...
    private string GetSQLWhere(string searchText)
    {
        ...
        {
            str_query += " WHERE ";
            str_query += $"(TitleNo LIKE '%{searchText}%' OR ";
    ...
    {
        string str_value = "";
        var model = repo.ReadSingle(m => m.TitleNo == dataNo);
        if (model != null) str_value = model.TitleName;
        return str_value;
    }
    /// <summary>
    /// 檢查 Id 是否存在
