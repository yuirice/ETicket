...
{
    /// <summary>
    /// MBASP002_ProgramType 程式類別資料維護
    /// </summary>
    public class MBASP002_ProgramTypeController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoCodeDatas repos = new z_repoCodeDatas())
            {
                PrgService.SearchText = searchText;
                PrgService.SetAction(enAction.Index, enCardSize.Max);
                PrgService.SetProgram();
                var model = repos.GetDapperDataList("Program", searchText).ToPagedList(page, pageSize);
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(CodeDatas model)
        {
