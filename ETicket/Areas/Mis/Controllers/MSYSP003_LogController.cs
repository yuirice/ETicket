...
{
    /// <summary>
    /// MSYSP003_Log 日誌記錄資料查詢
    /// </summary>
    public class MSYSP003_LogController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoLogs repos = new z_repoLogs())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Logs model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaLogs>(ModelState);
                return View(model);
            }
            using (z_repoLogs repos = new z_repoLogs())
            {
