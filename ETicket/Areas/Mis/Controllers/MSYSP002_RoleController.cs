...
{
    /// <summary>
    /// MSYSP002_RoleController
    /// </summary>
    public class MSYSP002_RoleController : BaseController
    {
        [HttpGet]
        [LoginAuthorize()]
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoRoles repos = new z_repoRoles())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Roles model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaRoles>(ModelState);
                return View(model);
            }
            using (z_repoRoles repos = new z_repoRoles())
            {
