...
{
    /// <summary>
    /// MSYSP001_Module 模組基本資料維護
    /// </summary>
    public class MSYSP001_ModuleController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoModules repos = new z_repoModules())
            {
        ...
        public ActionResult CreateEdit(int id = 0)
        {
            ...
            using (z_repoModules repos = new z_repoModules())
            {
                ...
                if (model == null)
                {
                    ...
                    {
                        model.IsEnabled = (bool)attr.GetDefaultValue<z_metaModules>("IsEnabled");
                        model.IsWorkflow = (bool)attr.GetDefaultValue<z_metaModules>("IsWorkflow");
                        model.RoleNo = (string)attr.GetDefaultValue<z_metaModules>("RoleNo");
                        model.SortNo = (string)attr.GetDefaultValue<z_metaModules>("SortNo");
                        model.ModuleNo = (string)attr.GetDefaultValue<z_metaModules>("ModuleNo");
                        model.ModuleName = (string)attr.GetDefaultValue<z_metaModules>("ModuleName");
                        model.IconName = (string)attr.GetDefaultValue<z_metaModules>("IconName");
                        model.Remark = (string)attr.GetDefaultValue<z_metaModules>("Remark");
                    }
                }
                return View(model);
            }
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Modules model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaModules>(ModelState);
                return View(model);
            }
            using (z_repoModules repos = new z_repoModules())
            {
