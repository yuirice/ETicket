...
{
    /// <summary>
    /// MBASP003_Language 語言基本資料維護
    /// </summary>
    public class MBASP003_LanguageController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoLanguages repos = new z_repoLanguages())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Languages model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaLanguages>(ModelState);
                return View(model);
            }
            using (z_repoLanguages repos = new z_repoLanguages())
            {
