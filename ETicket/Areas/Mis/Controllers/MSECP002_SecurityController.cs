...
{
    /// <summary>
    /// MSECP002_Security 使用者程式權限設定
    /// </summary>
    public class MSECP002_SecurityController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoUsers user = new z_repoUsers())
            {
                using (z_repoSecuritys repos = new z_repoSecuritys())
                {
                    ...
                    user.SetSubHeader(str_user_no, "使用者");
                    var model = repos.GetDapperDataList(searchText, "User", str_role_no, str_user_no).ToPagedList(page, pageSize);
            ...
            using (z_repoSecuritys repos = new z_repoSecuritys())
            {
                ...
                if (model == null)
                {
                    ...
                    model = new Securitys()
                    {
                        ...
                        TargetNo = SessionService.TagNo2,
                        IsAdd = true,
                        IsEdit = true,
                        IsConfirm = true,
                        IsInvalid = true,
