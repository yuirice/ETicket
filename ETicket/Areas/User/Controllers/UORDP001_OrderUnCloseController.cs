...
{
    /// <summary>
    /// UORDP001_OrderUnClose 未結訂單資料維護
    /// </summary>
    public class UORDP001_OrderUnCloseController : BaseController
    {
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoOrders repos = new z_repoOrders())
            {
                PrgService.SearchText = searchText;
                PrgService.SetAction(enAction.Index, enCardSize.Max);
                PrgService.SetProgram();
                var model = repos.GetDapperDataList(false, searchText).ToPagedList(page, pageSize);
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Orders model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaOrders>(ModelState);
                return View(model);
            }
            using (z_repoOrders repos = new z_repoOrders())
            {
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult Status(int id = 2)
        {
            using (z_repoOrders orders = new z_repoOrders())
            {
                PrgService.SetAction("訂單狀態變更", enCardSize.Small);
                SessionService.TagId1 = id; //把訂單 Id 先記起來
                var data = orders.repo.ReadSingle(m => m.Id == id);
                if (data == null) return RedirectToAction("Index", "UORDP001_OrderUnClose", new { area = "User" });
                OrdersStatus model = new OrdersStatus();
                model.StatusNo = data.StatusCode;
        ...
        /// 訂單狀態變更
        /// </summary>
        /// <param name="model">狀態資料</param>
        /// <returns></returns>
        [HttpPost]
        public ActionResult Status(OrdersStatus model)
        {
            using (z_repoOrders orders = new z_repoOrders())
            {
                var data = orders.repo.ReadSingle(m => m.Id == SessionService.TagId1);
                data.StatusCode = model.StatusNo;
                orders.repo.Update(data);
                orders.repo.SaveChanges();
                return RedirectToAction("Index", "UORDP001_OrderUnClose", new { area = "User" });
            }
        }
    }
}
