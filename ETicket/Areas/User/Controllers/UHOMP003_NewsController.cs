...
{
    /// <summary>
    /// UHOMP003_News 最新消息資料維護
    /// </summary>
    public class UHOMP003_NewsController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoNews repos = new z_repoNews())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(News model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaNews>(ModelState);
                return View(model);
            }
            using (z_repoNews repos = new z_repoNews())
            {
