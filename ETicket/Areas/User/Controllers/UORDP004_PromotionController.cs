...
namespace ETicket.Areas.User.Controllers
{
    ...
    public class UORDP004_PromotionController : BaseController
    {
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoPromotions repos = new z_repoPromotions())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Promotions model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaPromotions>(ModelState);
                return View(model);
            }
            using (z_repoPromotions repos = new z_repoPromotions())
            {
