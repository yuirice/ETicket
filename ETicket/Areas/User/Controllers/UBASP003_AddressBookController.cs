...
{
    /// <summary>
    /// UBASP003_AddressBook 個人通訊錄資料維護
    /// </summary>
    public class UBASP003_AddressBookController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoAddressBooks repos = new z_repoAddressBooks())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(AddressBooks model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaAddressBooks>(ModelState);
                return View(model);
            }
            using (z_repoAddressBooks repos = new z_repoAddressBooks())
            {
