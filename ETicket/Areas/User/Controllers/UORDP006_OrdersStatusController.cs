...
{
    /// <summary>
    /// UORDP006_OrdersStatus 訂單狀態資料維護
    /// </summary>
    public class UORDP006_OrdersStatusController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoOrdersStatus repos = new z_repoOrdersStatus())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(OrdersStatus model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaOrdersStatus>(ModelState);
                return View(model);
            }
            using (z_repoOrdersStatus repos = new z_repoOrdersStatus())
            {
