...
namespace ETicket.Areas.User.Controllers
{
    ...
    public class UORDP003_ProductController : BaseController
    {
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoProducts repos = new z_repoProducts())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Products model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaProducts>(ModelState);
                return View(model);
            }
            using (z_repoProducts repos = new z_repoProducts())
            {
