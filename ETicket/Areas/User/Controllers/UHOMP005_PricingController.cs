...
{
    /// <summary>
    /// UHOMP005_Pricing 商品價格資料維護
    /// </summary>
    public class UHOMP005_PricingController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoPricings repos = new z_repoPricings())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Pricings model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaPricings>(ModelState);
                return View(model);
            }
            using (z_repoPricings repos = new z_repoPricings())
            {
