...
namespace ETicket.Areas.User.Controllers
{
    ...
    /// UBASP002_CityArea
    /// </summary>
    public class UBASP002_CityAreaController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoCityAreas repos = new z_repoCityAreas())
            {
                PrgService.SearchText = searchText;
                PrgService.SetAction(enAction.Index, enCardSize.Max);
                var model = repos.GetDapperDataList(SessionService.TagName1, searchText).ToPagedList(page, pageSize);
        ...
        public ActionResult CreateEdit(int id = 0)
        {
            ...
            using (z_repoCityAreas repos = new z_repoCityAreas())
            {
                ...
                if (model == null)
                {
                    model = new CityAreas()
                    {
                        CityName = SessionService.TagName1,
                        Remark = ""
                    };
                }
                return View(model);
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(CityAreas model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaCodeDatas>(ModelState);
                return View(model);
            }
            using (z_repoCityAreas repos = new z_repoCityAreas())
            {
                model.CityName = SessionService.TagName1;
