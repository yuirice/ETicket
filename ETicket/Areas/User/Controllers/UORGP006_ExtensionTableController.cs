...
{
    /// <summary>
    /// UORGP006_ExtensionTable 公司分機表資料維護
    /// </summary>
    public class UORGP006_ExtensionTableController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoExtensionTables repos = new z_repoExtensionTables())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(ExtensionTables model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaExtensionTables>(ModelState);
                return View(model);
            }
            using (z_repoExtensionTables repos = new z_repoExtensionTables())
            {
