...
{
    /// <summary>
    /// UORDP008_Shipping 出貨方式資料維護
    /// </summary>
    public class UORDP008_ShippingController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoShippings repos = new z_repoShippings())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Shippings model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaShippings>(ModelState);
                return View(model);
            }
            using (z_repoShippings repos = new z_repoShippings())
            {
