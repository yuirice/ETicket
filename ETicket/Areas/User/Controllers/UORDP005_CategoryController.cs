...
{
    /// <summary>
    /// UORDP005_Product 商品分類資料維護
    /// </summary>
    public class UORDP005_CategoryController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        [HttpGet]
        [LoginAuthorize()]
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "", int parentId = 0)
        {
            //檢查瀏覽權限
        ...
        public ActionResult Detail(int id = 0)
        {
            using (z_repoCategorys repos = new z_repoCategorys())
            {
                ...
                return RedirectToAction(ActionService.Index, ActionService.Controller, new { area = ActionService.Area, parentId = id });
            }
        }
        ...
        public ActionResult CreateEdit(int id = 0)
        {
            ...
            using (z_repoCategorys repos = new z_repoCategorys())
            {
                ...
                if (model == null)
                {
                    if (string.IsNullOrEmpty(SessionService.TagNo1)) SessionService.TagNo1 = "";
                    // 設定新增預設值
                    model = new Categorys()
                    {
                        ParentNo = SessionService.TagNo1,
                        Remark = ""
                    };
                }
                return View(model);
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Categorys model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaCategorys>(ModelState);
                return View(model);
            }
            using (z_repoCategorys repos = new z_repoCategorys())
            {
                model.ParentNo = SessionService.TagNo1;
                repos.CreateEdit(model);
                return RedirectToAction(ActionService.Index, ActionService.Controller, new { area = ActionService.Area, parentId = SessionService.TagId1 });
            }
        }
