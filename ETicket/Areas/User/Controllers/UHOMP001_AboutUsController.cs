...
{
    /// <summary>
    /// UHOMP001_AboutUs 公司介紹資料維護
    /// </summary>
    public class UHOMP001_AboutUsController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoAboutUs repos = new z_repoAboutUs())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(AboutUs model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaClients>(ModelState);
                return View(model);
            }
            using (z_repoAboutUs repos = new z_repoAboutUs())
            {
