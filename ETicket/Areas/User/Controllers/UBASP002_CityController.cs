...
{
    /// <summary>
    /// UBASP002_City 縣市基本資料維護
    /// </summary>
    public class UBASP002_CityController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoCitys repos = new z_repoCitys())
            {
        ...
        public ActionResult Detail(int id = 0)
        {
            using (z_repoCitys repos = new z_repoCitys())
            {
                ...
                SessionService.TagName1 = data.CityName;
                PrgService.SubHeader = $"{data.CityName}";
                return RedirectToAction(ActionService.Index, "UBASP002_CityArea", new { area = ActionService.Area });
            }
        }
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Citys model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaCitys>(ModelState);
                return View(model);
            }
            using (z_repoCitys repos = new z_repoCitys())
            {
