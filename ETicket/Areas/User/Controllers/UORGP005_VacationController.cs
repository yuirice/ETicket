...
{
    /// <summary>
    /// UORGP005_Vacation 公司休假行事曆維護
    /// </summary>
    public class UORGP005_VacationController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoVacations repos = new z_repoVacations())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Vacations model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaVacations>(ModelState);
                return View(model);
            }
            using (z_repoVacations repos = new z_repoVacations())
            {
