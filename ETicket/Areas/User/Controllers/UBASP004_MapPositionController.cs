...
{
    /// <summary>
    /// UBASP004_MapPosition 地圖地點資料維護
    /// </summary>
    public class UBASP004_MapPositionController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoMapPositions repos = new z_repoMapPositions())
            {
        ...
        public ActionResult CreateEdit(int id = 0)
        {
            ...
            using (z_repoMapPositions repos = new z_repoMapPositions())
            {
                ...
                {
                    // 設定新增預設值
                    model = new MapPositions()
                    {
                        TargetCode = "User",
                        TargetNo = UserService.UserNo,
                        Remark = ""
                    };
                }
                return View(model);
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(MapPositions model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaMapPositions>(ModelState);
                return View(model);
            }
            using (z_repoMapPositions repos = new z_repoMapPositions())
            {
