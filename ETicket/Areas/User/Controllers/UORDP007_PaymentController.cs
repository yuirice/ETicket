...
{
    /// <summary>
    /// UORDP007_Payment 付款方式資料維護
    /// </summary>
    public class UORDP007_PaymentController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoPayments repos = new z_repoPayments())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Payments model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaPayments>(ModelState);
                return View(model);
            }
            using (z_repoPayments repos = new z_repoPayments())
            {
