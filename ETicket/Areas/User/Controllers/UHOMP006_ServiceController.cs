...
{
    /// <summary>
    /// UHOMP006_Service 公司服務資料維護
    /// </summary>
    public class UHOMP006_ServiceController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, int pageSize = 10, string searchText = "")
        {
            ...
            using (z_repoServices repos = new z_repoServices())
            {
        ...
        [HttpPost]
        [LoginAuthorize()]
        public ActionResult CreateEdit(Services model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metaServices>(ModelState);
                return View(model);
            }
            using (z_repoServices repos = new z_repoServices())
            {
